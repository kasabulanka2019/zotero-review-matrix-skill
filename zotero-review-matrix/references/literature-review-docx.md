...
Write a literature review in Word `.docx` format from the completed Zotero review matrix. The review should synthesize arguments and evidence rather than list papers one by one.

## Required Structure

Use a clear Chinese academic prose structure by default:
...
- Preserve uncertainty from the matrix. Use cautious language when evidence comes only from abstracts or thin notes.
...
- Create a `.docx` file unless the user requests only Markdown.
- Use readable heading levels.
- Put `参考文献` at the end.
- Ensure every in-text cited work appears in the reference list when metadata is available.
- Do not cite works in the reference list that were not part of the collection evidence base unless the user explicitly adds them.
